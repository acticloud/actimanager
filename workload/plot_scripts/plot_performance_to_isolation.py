...
#	ax.plot(vm_event_times[k], vm_perfs[k], color=color, marker=marker)
#	if len(vm_event_times[k]) > 0:
#		ax.text(vm_event_times[k][0], vm_perfs[k][0], str(k), color=color)
#
...
for i in range(4):
#	axs[i].axhline(y=1.0)
	axs[i].axhline(y=0.8, ls='--')
	axs[i].set_ylim(0.0, 1.1)
#	axs[i].set_title("Acticloud" + str(i+1))
for k in vm_perfs:
	marker = 'o' if k in gold_vms else 'x'
#	color = "yellow"
#	if "spec" in vms_names[k]:
#		color = "red"
#	elif "stream" in vms_names[k]:
#		color = "green"
	base_color = 'gold' if k in gold_vms else 'silver' ## Get base color
	color = matplotlib.colors.to_rgba(base_color)
	color = list(color)
...
#		ax.plot(vm_event_times[k], vm_perfs[k], color=color, marker=marker)
	ax.plot(vm_event_times[k], vm_perfs[k], color=color, marker=marker)
#	if len(vm_event_times[k]) > 0 and k in gold_vms:
#		ax.text(vm_event_times[k][0], vm_perfs[k][0] + 0.05, vms_names[k], color=color)
